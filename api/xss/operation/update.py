...
class CrossSiteScriptingRuleUpdate(Resource):
    def put(self, id):
        if id is None:
            return {
                'type': 'xss',
        ...
        psql_cursor.execute(f"SELECT * FROM xss WHERE id = '{id}';")
        result = psql_cursor.fetchone()
        if result is None:
            return {
                'type': 'xss',
                'data': None,
                'reason': 'Cross Site Scripting Rule is not found for update'
            }, 404
        ...
        if old_rule_name != request_body.get('ruleName'):
            psql_cursor.execute("SELECT rule_name FROM xss WHERE rule_name = '{rule_name}';".format(rule_name=request_body['ruleName']))
            rows = psql_cursor.fetchall()
            if rows.__len__() > 0:
                return {
                    'type': 'xss',
